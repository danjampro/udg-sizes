...
def pretty_plot(rec_phys, popt, pcov, bins=10, nsamples=500, fontsize=15, filename=None):
    ...
    # ax.plot(centres, y, "bo", fillstyle="none", markeredgewidth=1.5)
    ...
    ax.plot(np.log10(xx), yyb, "--", color="orangered", linewidth=1.5, label="vdB+16 (clusters)")

    # yybest = udg_powerlaw(xx, *popt)
    # ax.plot(np.log10(xx), yybest, "--", color="r")
